...
#Lastly, a term frequency–inverse document frequency model is created to show the importance of words in a corpus
...
tips = tips.merge(business[["city", "state", "industry", "business_id"]], how = "left",
...
# Remove punctuation
tips['text_clean'] = \
tips['text'].map(lambda x: re.sub(r'[,\/.!?-]', '', x)) 


# Convert the titles to lowercase
tips['text_clean'] = \
tips['text_clean'].map(lambda x: x.lower())

tips['text_clean'].head()
...
data = tips.text_clean.values.tolist()
...
# Print keywords in the 6 topics of tips
...
st.set_page_config(layout="centered", page_icon="👔", page_title="Yelp Business Tool")
...
def get_reviews_topics(state, city, industry):
  tips_f = tips[tips["industry"] == industry & tips["city"] == city & tips["state"] == state]
  tips_f['text_clean'] = tips_f['text'].tolist()
  tips_f['text_clean'] = \
  tips_f['text'].map(lambda x: re.sub(r'[,\/.!?-]', '', x))
  tips_f['text_clean'] = \
  tips_f['text_clean'].map(lambda x: x.lower())
  nltk.download('stopwords')
  stop_words = stopwords.words('english')
  data_tips = tips_f.text_clean.values.tolist()
